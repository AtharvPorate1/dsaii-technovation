export const eventStartDate = 'April 1, 2024 10:00:00';

export const coordinators = [
  { "name": "Sanika Karulkar", "roll": "President", "phone": "", "email": "" },
  { "name": "Gourisha Verma", "roll": "Vice President", "phone": "", "email": "" },
  { "name": "Gayatri Khirsagar", "roll": "Secretary", "phone": "", "email": "" },
  { "name": "Aaditya Khairnar", "roll": "Joint Secretary", "phone": "", "email": "" },
  { "name": "Rohit Kalaburgi", "roll": "Treasurer", "phone": "", "email": "" },
  { "name": "Gunjan Potdukhe", "roll": "Marketing Head", "phone": "", "email": "" },
  { "name": "Yash Thakur", "roll": "Technical Head", "phone": "", "email": "" },
  { "name": "Saamya Daryanani", "roll": "Creative Head", "phone": "", "email": "" },
  { "name": "Prajakta Gaikwad", "roll": "Cultural Head", "phone": "", "email": "" },
  { "name": "Dhruv Pai", "roll": "Event Management Head", "phone": "", "email": "" },
  { "name": "Dhruv Yaranalkar", "roll": "", "phone": "", "email": "" },
    { "name": "Sarfaraz Siddiqui", "roll": "", "phone": "", "email": "" },
    { "name": "Yash Khadyate", "roll": "", "phone": "", "email": "" },
    { "name": "Bishal Roy", "roll": "", "phone": "", "email": "" },
    { "name": "Aditya Mahindrakar", "roll": "", "phone": "", "email": "" },
    { "name": "Vishal Salunke", "roll": "", "phone": "", "email": "" },
    { "name": "Kashish Jain", "roll": "", "phone": "", "email": "" },
    { "name": "Sayali Chavan", "roll": "", "phone": "", "email": "" },
    { "name": "Yash Shinkar", "roll": "", "phone": "", "email": "" },
    { "name": "Vidit Solanki", "roll": "", "phone": "", "email": "" },
    { "name": "Gargee Rangnekar", "roll": "", "phone": "", "email": "" },
    { "name": "Atharva Khollam", "roll": "", "phone": "", "email": "" },
    { "name": "Varad Yeldarkar", "roll": "", "phone": "", "email": "" },
    { "name": "Shreyas Deshmukh", "roll": "", "phone": "", "email": "" },
    { "name": "Omkar Yadav", "roll": "", "phone": "", "email": "" },
    { "name": "Aneri Patel", "roll": "", "phone": "", "email": "" },
    { "name": "Niranjan Chirde", "roll": "", "phone": "", "email": "" },
    { "name": "Rutuja Jain", "roll": "", "phone": "", "email": "" },
    { "name": "Narayani Sunil Gupta", "roll": "", "phone": "", "email": "" },
    { "name": "Payal Sarode", "roll": "", "phone": "", "email": "" },
 
   
]
;

export const mainCoordinators = [
  {
    "name": "Sanika Karulkar",
    "imgSrc": "/path/to/image1.jpg",
    "roles": ["President"]
  },
  {
    "name": "Gourisha Verma",
    "imgSrc": "/path/to/image2.jpg",
    "roles": ["Vice President"]
  },
  {
    "name": "Gayatri Khirsagar",
    "imgSrc": "/path/to/image3.jpg",
    "roles": ["Secretary"]
  },
  {
    "name": "Aaditya Khairnar",
    "imgSrc": "/path/to/image4.jpg",
    "roles": ["Joint Secretary"]
  },
  {
    "name": "Rohit Kalaburgi",
    "imgSrc": "/path/to/image5.jpg",
    "roles": ["Treasurer"]
  },
  {
    "name": "Gunjan Potdukhe",
    "imgSrc": "/path/to/image6.jpg",
    "roles": ["Marketing Head"]
  },
  {
    "name": "Yash Thakur",
    "imgSrc": "/path/to/image7.jpg",
    "roles": ["Technical Head"]
  },
  {
    "name": "Saamya Daryanani",
    "imgSrc": "/path/to/image8.jpg",
    "roles": ["Creative Head"]
  },
  {
    "name": "Prajakta Gaikwad",
    "imgSrc": "/path/to/image9.jpg",
    "roles": ["Cultural Head"]
  },
  {
    "name": "Dhruv Pai",
    "imgSrc": "/path/to/image10.jpg",
    "roles": ["Event Management Head"]
  }
]
;

export const highlights = ['0', '1','2','3','4'];

export const events = {
  "0": {
    "id": "0",
    "title": "TechNova",
    "figureSrc": "/media/events/technova.jpeg.jpg",
    "day": 0,
    "time": "10:00 - 11:00",
    "venue": "Welcome to TechNova, our flagship event that celebrates vision and technical excellence! TechNova is not just a competition; it's a vibrant platform where creative minds from different domains come together to showcase their projects to expert judges. Whether you're into artificial intelligence, cybersecurity, software development, hardware design, or any other domain, TechNova is the place for you. We believe in fostering collaboration, pushing boundaries, and shaping the future through smart solutions. Join us on this exciting journey as we explore multiple domains and pave the way for a brighter tomorrow at TechNova!",
    "desc": "",
    "type": "Technical",
    "highlight": false
  },
  "1": {
    "id": "1",
    "title": "Diplomatrix",
    "figureSrc": "/media/events/diplo.jpeg.jpg",
    "day": 0,
    "time": "10:00 - 11:00",
    "venue": "At Diplomatrix, we take case studies of different AI products widely used today and dive into solving the issues and conflicts surrounding these products. The event unfolds in three phases: Agenda Presentation, Collaboration and Resolution Drafting, and Resolution Presentation to expert judges. Teams collaborate to draft resolutions addressing governance issues and ethical considerations, which are then evaluated based on clarity, coherence, depth of research, feasibility of recommendations, and team collaboration. Join us at Diplomatrix for a transformative exploration of AI governance, where formal dress code adds to the professional atmosphere!",
    "desc": "",
    "type": "Technical",
    "highlight": false
  },
  "2": {
    "id": "2",
    "title": "Bug Buster",
    "figureSrc": "/media/events/bug3.jpeg.jpg",
    "day": 0,
    "time": "10:00 - 11:00",
    "venue": "Welcome to Bug Busters, where the code debugging champions are born! Bug Busters is a thrilling two-round contest designed to put your error correction skills to the ultimate test",
    "desc": "",
    "type": "Technical",
    "highlight": false
  },
  "3": {
    "id": "3",
    "title": "Suit-Up: Thematic Case Competition",
    "figureSrc": "/media/events/band.jpg",
    "day": 0,
    "time": "10:00 - 11:00",
    "venue": "Dive into an engaging event where participants, divided into two groups, choose from thematic cases like Stranger Things and Harry Potter. The audience acts as the jury as teams compete for victory with thoughtfully crafted storylines. A comprehensive rule book ensures fairness, and participants showcase persuasive skills without AI tools. Join us for a short and competitive platform where advocacy prowess shines!",
    "desc": "",
    "type": "Non-Technical",
    "highlight": false
  },
  "4": {
    "id": "4",
    "title": "Nerf Arena: Foam Dart Battle",
    "figureSrc": "/media/events/band.jpg",
    "day": 0,
    "time": "10:00 - 11:00",
    "venue": "Engage in thrilling battles as two teams face off amid obstacles, barriers, and hiding spots. Participants must use their wits and blaster skills to outmaneuver and outgun opponents. The intense and exciting battle features foam darts flying fast and furious. Team up, strategize, and coordinate attacks to hit the enemy target for victory! Get ready for an adrenaline-pumping experience in the NERF ARENA!",
    "desc": "",
    "type": "Non-Technical",
    "highlight": false
  },
  "5": {
    "id": "4",
    "title": "Nerf Arena: Foam Dart Battle",
    "figureSrc": "/path/to/image5.jpg",
    "day": 1,
    "time": "10:00 - 11:00",
    "venue": "Venue 1",
    "desc": "Engage in thrilling battles as two teams face off amid obstacles, barriers, and hiding spots. Participants must use their wits and blaster skills to outmaneuver and outgun opponents. The intense and exciting battle features foam darts flying fast and furious. Team up, strategize, and coordinate attacks to hit the enemy target for victory! Get ready for an adrenaline-pumping experience in the NERF ARENA!",
    "type": "Non-Technical",
    "highlight": false
  }
}


export const eventSlots = {
  '0': ['EventID1', 'EventID2'],
  '1': ['EventID3', 'EventID4'],
  // Add more event slots here with similar structure
}
